...
| `assets/css/extended/custom.css` | Everything else, organized into 13 numbered sections (§0–§12): **§0** font-faces (Karla body + Rubik headings + JetBrains Mono code, all variable `woff2`, self-hosted in `static/fonts/`); **§1** design tokens (`--accent`/`--accent-2`, full light+dark palette, `--font-body`/`--font-heading`/`--font-mono`, `--main-width`, `--measure`, plus a commented copy of PaperMod's default theme-vars kept for palette-planning reference — **`--code-bg` must stay a neutral surface**: PaperMod also uses it for list-page backgrounds and the series-nav box); **§2** typography (narrow-prose/wide-media, accent headings, text-wrap; tables get tabular digits, a 12%-accent header fill with 2px accent underline, and an 8%-accent row-hover tint with a 3px inset accent edge on the first cell); **§3** media (image borders/shadows, lightbox at 95vw/95vh with a wider `img[src$=".svg"]` rule since vectors have no intrinsic size); **§4** code (mono font applied to blocks + inline, inline-code 14%-accent tint + hairline via `:not(pre) > code`, hover accent ring + shadow on `.code-wrap`, language labels); **§5** reading aids (progress bar, TOC); **§6** post-list cards (hover lift; `.entry-content` bumped to 15px); **§7** cover images (list/thumbnail/single); **§8** series navigation; **§9** nav + tags (animated underline); **§10** profile/landing page; **§11** misc embeds; **§12** a site-wide `prefers-reduced-motion` guard (kept last). Note: the lightbox "Download full image" link is built in `extend_footer.html` JS and has no dedicated CSS rule here. |
...
- **Fonts (Karla / Rubik / JetBrains Mono)** → `custom.css` (§0 `@font-face` + §1 `--font-body` / `--font-heading` / `--font-mono`) + `static/fonts/`
